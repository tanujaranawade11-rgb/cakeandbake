...
/**
 * Servlet implementation class AddCake_servlet
 */
@WebServlet("/AddCake_servlet")
@MultipartConfig
public class AddCake_servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AddCake_servlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(cakeDaoimpl.addCake(cakePojo)) {
			session.setAttribute("msg","Cake add successfully!!");
			response.sendRedirect("admin.jsp");
		}
		else {
			session.setAttribute("msg","Cake not add successfully!!");
			response.sendRedirect("admin.jsp");

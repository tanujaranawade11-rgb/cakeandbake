...
/**
 * Servlet implementation class Addprofile_servlet
 */
@WebServlet("/Addprofile_servlet")
@MultipartConfig
public class Addprofile_servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public Addprofile_servlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(profileImpl.addProfile(profilepojo)) {
			session.setAttribute("msg","profile added successfully!!");
			response.sendRedirect("profile.jsp");
		}
		else {
			session.setAttribute("msg","profile not added successfully!!");
			response.sendRedirect("profile.jsp");
		}
	}
